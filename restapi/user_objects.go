...
func getDeleteMultiplePathsResponse(session *models.Principal, params user_api.DeleteMultipleObjectsParams) *models.Error {
	ctx := context.Background()
	var version string
	for i := 0; i < len(params.PathName); i++ {
		if params.VersionID != nil {
			version = *params.VersionID
		}
		s3Client, err := newS3BucketClient(session, params.BucketName, params.PathName[i])
		if err != nil {
			return prepareError(err)
		}
		// create a mc S3Client interface implementation
		// defining the client to be used
		mcClient := mcClient{client: s3Client}
		err = deleteObjects(ctx, mcClient, params.BucketName, params.PathName[i], version, params.Recursive[i])
		if err != nil {
			return prepareError(err)
		}
	}
