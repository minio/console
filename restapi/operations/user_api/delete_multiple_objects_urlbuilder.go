...
func (o *DeleteMultipleObjectsURL) Build() (*url.URL, error) {
	...
	var _path = "/buckets/{bucket_name}/delete-multiple/{path_name}"
	...
	for _, pathNameI := range o.PathName {
		pathNameIS := pathNameI
		if pathNameIS != "" {
			pathNameIR = append(pathNameIR, pathNameIS)
		}
	}

	pathName := swag.JoinByFormat(pathNameIR, "")
	if len(pathName) > 0 {
		psv := pathName[0]
		if psv != "" {
			_path = strings.Replace(_path, "{path_name}", psv, -1)
		} else {
			return nil, errors.New("pathName is required on PathNameURL")
		}
	}
	...
	var recursiveIR []string
	for _, recursiveI := range o.Recursive {
		recursiveIS := swag.FormatBool(recursiveI)
		if recursiveIS != "" {
			recursiveIR = append(recursiveIR, recursiveIS)
		}
	}

	recursive := swag.JoinByFormat(recursiveIR, "")

	if len(recursive) > 0 {
		qsv := recursive[0]
		if qsv != "" {
			qs.Set("recursive", qsv)
		}
	}
