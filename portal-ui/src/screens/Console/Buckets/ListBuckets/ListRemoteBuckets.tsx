...
interface IListRemoteBucketsProps {
  classes: any;
...
const ListRemoteBuckets = ({
  classes,
  addBucketOpen,
  addBucketModalOpen,
  addBucketReset,
  setErrorSnackMessage,
}: IListRemoteBucketsProps) => {
  const [records, setRecords] = useState<RemoteBucket[]>([]);
  ...
  const filteredRecords = records.filter((b: RemoteBucket) => {
    if (filterBuckets === "") {
      return true;
    } else {
      if (b.sourceBucket.indexOf(filterBuckets) >= 0) {
        return true;
      } else {
        return false;
      }
          ...
          <Grid item xs={12}>
            <TableWrapper
              ...
              columns={[
                ...
                {
                  label: "Target Bucket",
                  elementKey: "targetBucket",
                },
                {
                  label: "Target URL",
                  elementKey: "targetURL",
                },
              ]}
              isLoading={loading}
              records={filteredRecords}
              entityName="Remote Buckets"
              idField="sourceBucket"
...
export default connector(withStyles(styles)(ListRemoteBuckets));
