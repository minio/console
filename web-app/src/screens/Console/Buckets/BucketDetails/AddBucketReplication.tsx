...
const AddBucketReplication = () => {
  ...
  // );
  //const checkValidURL = (testURL: string) => {
  //console.log("Tested URL: ", validURL.test(testURL));
  //return validURL.test(testURL);
  const checkValidURL = (testURL: string) => {
    const endURL = testURL.startsWith("https://" || "http://")
      ? testURL
      : `${useTLS ? "https://" : "http://"}${testURL}`;
    try {
      new URL(endURL);
      return true;
    } catch (err) {
  ...
  useEffect(() => {
    !validated &&
      ...
      setValidated(true);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [targetURL, accessKey, secretKey, targetBucket]);

  useEffect(() => {
    if (
      (validated && accessKey.length < 3) ||
      secretKey.length < 8 ||
      targetBucket.length < 3 ||
      !checkValidURL(targetURL)
    ) {
